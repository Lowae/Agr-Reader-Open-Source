...
class ExternalFonts(
    ...
    fun copyToInternalStorage(inputStream: InputStream, onSuccess: () -> Unit) {
    ...
    companion object {
        ...
        fun loadReadingTypography(ctx: Context): Typography =
            loadTypography(ctx, FontType.ReadingFont)

        fun loadBasicFontFile(ctx: Context = AgrReaderApp.application) = File(FontType.BasicFont.toPath(ctx))
        ...
        private fun loadTypography(ctx: Context, type: FontType): Typography =
            when (type) {
                FontType.BasicFont -> basicTypography ?: createTypography(
                    createFontFamily(
                        ctx,
                        type
                    )
                ...
                FontType.ReadingFont -> readingTypography ?: createTypography(
                    createFontFamily(
                        ctx,
                        type
                    )
